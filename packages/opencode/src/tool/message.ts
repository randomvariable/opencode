  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("MessageTool.execute")(function* (
      ...
      if (params.target === "subagent") {
        if (!params.task_id) return yield* Effect.fail(new Error('message(target:"subagent") requires task_id'))
      ...
      if (useChannelB && !ops) return yield* Effect.fail(new Error("message tool requires promptOps in ctx.extra"))
      ...
      const deliver: Effect.Effect<void> =
        expectReply && parked ? background.message(ctx.sessionID, payload).pipe(Effect.asVoid) : inject().pipe(Effect.orDie)
