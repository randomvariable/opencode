...
const Parameters = Schema.Struct({
  ...
  server: Schema.optional(Schema.String).annotate({
    description: "MCP server name (required for 'describe' and 'call')",
  }),
  tool: Schema.optional(Schema.String).annotate({
    description: "Tool name on that server (required for 'describe' and 'call')",
  }),
    ...
    return {
      ...
      execute(params: McpSearchParams, ctx: Tool.Context): Effect.Effect<Tool.ExecuteResult<Record<string, unknown>>> {
        ...
        return Effect.gen(function* () {
          ...
          if (!server || !tool)
            throw new Error(
              `Both 'server' and 'tool' parameters are required. Received: server=${JSON.stringify(server)}, tool=${JSON.stringify(tool)}. Use parameter names "server" and "tool", not "mcp_name" or "tool_name".`,
            )
