...
export function processMcpResult(result: {
  ...
  const { text, attachments } = result.content.reduce(
    (acc, item) => {
      ...
      if (item.type === "image")
        acc.attachments.push({ type: "file", mime: item.mimeType!, url: `data:${item.mimeType};base64,${item.data}` })
      if (item.type === "resource") {
        ...
        if (r.blob)
          acc.attachments.push({
            ...
            url: `data:${r.mimeType ?? "application/octet-stream"};base64,${r.blob}`,
            filename: r.uri,
          })
      }
      return acc
    },
    { text: [] as string[], attachments: [] as Omit<SessionV1.FilePart, "id" | "sessionID" | "messageID">[] },
  )
