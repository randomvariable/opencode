...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    if (credential.auth === "wellknown") {
      yield* Effect.orDie(authWellKnown.remove(credential.key))
    }
    if (credential.auth === "provider") {
      yield* Effect.orDie(authSvc.remove(credential.key))
    }
