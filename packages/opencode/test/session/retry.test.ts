...
describe("session.message-v2.fromError", () => {
  ...
  test("extracts embedded OpenAI stream errors from validation wrapper text", () => {
    ...
    if (!SessionV1.APIError.isInstance(result)) throw new Error("expected APIError")
    expect(result.data.isRetryable).toBe(true)
    // The fork's retryable() returns the provider message verbatim for APIError
    // instances (it does not re-parse responseBody like upstream); the envelope
    // is still treated as retryable, which is the behaviour this PR targets.
    expect(SessionRetry.retryable(result, retryProvider)).toEqual({ message: body.error.message })
  })
})
